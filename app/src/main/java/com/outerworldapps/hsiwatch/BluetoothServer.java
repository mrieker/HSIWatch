        ...
        for (ReceiveThread rt : rts) {
            if (! rt.senderr) {
                try {
                    ...
                    Log.w (MainActivity.TAG, "error sending to bluetooth", ioe);
                    rt.senderr = true;
                }
            }
        }
    ...
    private class AcceptThread extends Thread {
        ...
        public void run ()
        {
            try {
                ...
                Log.w (MainActivity.TAG, "error accepting bluetooth " + sppUUID, e);
                try { serverSocket.close (); } catch (Exception ignored) { }
                serverSocket = null;
                if (! finished) {
                    udpMainPage.mainActivity.runOnUiThread (new Runnable () {
                        @Override
                        public void run ()
                        {
                            udpMainPage.mainActivity.showToastLong ("Bluetooth Error\n" +
        ...
        public void run ()
        {
            try {
                ...
                synchronized (connectionLock) {
                    ...
                    udpMainPage.updateConCount (rtarray.length);
                }
                ...
                Log.w (MainActivity.TAG, "error receiving from bluetooth", ioe);
            } finally {
