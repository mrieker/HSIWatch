...
public class TextArraySpinner extends Button implements View.OnClickListener {
    ...
    public interface OnItemSelectedListener {
        boolean onItemSelected (View view, int index, String text);
    ...
    {
        return (index < 0) ? unlabels[~index] : labels[index];
    }
    ...
    {
        int i = 0;
        for (String unlabel : unlabels) {
            -- i;
            if (text.equals (unlabel)) {
                setIndex (i);
                return true;
            }
        }
    ...
    private void selected (int i)
    {
        Lib.dismiss (dialog);
        dialog = null;
        if ((clicked == null) || clicked.onItemSelected (this, i, labels[i])) {
            setIndex (i);
        }
    }
